...
export const BlogComponent = () => {
  ...
  const { data } = useSWR<resObject>('/api/youtube_videos?pageToken=' + pageToken, fetcher);
  ...
  return (<div>
    <div className="flex flex-col items-start justify-center max-w-2xl mx-auto mb-16 mt-10">
      ...
      {searchValue && (
        filteredBlogPosts && filteredBlogPosts.map((post: video) => (
      ...
      <Suspense fallback={null}>
        <div className="flex flex-row">
          ...
          <div className="absolute right-10">
            {data && data.prevPageToken && <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={() => setPageToken(data?.prevPageToken ?? "")}>Prev</button>}
            {data && data.nextPageToken && <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={() => setPageToken(data?.nextPageToken ?? "")}>Next</button>}
          </div>
        </div>
        {!posts && (
          <p className="mb-4 text-gray-600 dark:text-gray-400">
